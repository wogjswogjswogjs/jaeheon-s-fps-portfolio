...
namespace _2.Scripts.Player
{
    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        private bool IsGrounded()
        {
            ...
            Debug.DrawRay(playerComponents.playerTransform.position + Vector3.up * 2 * playerValues.colliderExtents.x,Vector3.down,Color.red);
        ...
        private void FixedUpdate()
        {
            ...
            if (LockedBehaviourCode > 0 || overrideBehaviours.Count == 0)
            {
                foreach (BaseBehaviour behaviour in behaviours)
                {
                    ...
                    Debug.Log($"<color=red> current code : {currentBehaviourCode} </color>");
                    Debug.Log($"<color=blue> GetBehaviourCode code : {behaviour.GetBehaviourCode} </color>");
